...
class Extractor():
    ...
    def getRawPage(self):
        ...
        charset = re.compile(
            r'content="text/html;.?charset=(.*?)"').findall(str(resp.content))
    ...
    def processTags(self):
        ...
        self.body = re.sub(reTRIM.format("script"), "", re.sub(
            reTRIM.format("style"), "", self.body))
    ...
    def processBlocks(self):
        ...
        for i in range(self.blockSize):
            self.cblocks = list(
                map(lambda x, y: x+y, self.textLens[i: lines-1-self.blockSize+i], self.cblocks))
...
if __name__ == '__main__':
    ext = Extractor(
        url="http://business.sohu.com/20140629/n401582189.shtml", blockSize=6, image=False)
