// tcc -lpng -lavformat -lavcodec -lavutil -run waveform.c test.mp4
// ref:
//	1.https://github.com/rzurad/waveform/blob/master/main.c
//  2./usr/share/ffmpeg/examples/demuxing_decoding.c
 ...
 */
static void read_raw_audio_data(AudioData *data, int populate_sample_buffer, int audio_stream_index)
{
    ...
    {
        // TODO: replace total bitrate[format_context->bit_rate] with audio bitrate
    ...
    if (!data->samples)
    {
        puts("malloc failed, data is too large.");
        return;
    }
    ...
    {
        printf("\rframe number=%d", frame_index++);
            ...
            if (frame_finished)
            {
                ...
                if (is_planar)
                {
                    ...
                    for (; i < data_size / data->channels; i += data->sample_size)
                    {
                        for (c = 0; c < data->channels; c++)
                        {
                            if (data->samples && populate_sample_buffer && pFrame->extended_data[c])
                            {
 ...
 * `read_audio_metadata`.
 */
void read_audio_data(AudioData *data, int stream_index)
{
    read_raw_audio_data(data, 1, stream_index);
}
 ...
 */
void read_audio_metadata(AudioData *data, int stream_index)
{
    read_raw_audio_data(data, 0, stream_index);
}
...
{
    int width = 4096;                  // default width of the generated png image
    int height = 512;                   // default height of the generated png image
    ...
    const char *pFilePath = argv[1];   // audio input file path
    const char *pOutFile = "test.png"; // image output file path. `NULL` means stdout
    ...
    while ((c = getopt(argc, argv, "c:b:i:o:dmw:h:t:")) != -1)
    {
        ...
        {
        case 'b':
            read_color(strtol(optarg, NULL, 16), &color_bg[0]);
            break;
        case 'c':
            read_color(strtol(optarg, NULL, 16), &color_waveform[0]);
            break;
        case 'd':
            metadata = 1;
            break;
    ...
    AVStream *st = pFormatContext->streams[stream_index];

    AVCodec *dec = avcodec_find_decoder(st->codecpar->codec_id);

    // now that we have a stream, get the codec for the given stream
    pDecoderContext = avcodec_alloc_context3(dec);

    avcodec_parameters_to_context(pDecoderContext, st->codecpar);
    ...
    if (metadata)
    {
        ...
        read_audio_metadata(data, stream_index);
    ...
    else
    {
        ...
        read_audio_data(data, stream_index);
