...
Matrix inv(const Matrix &m)
{
	...
	for (int k = 0; k < n; k++)
	{
		...
		for (int j = 0; j < n; j++)
			if (j != k)
				m.elems[k][j] = m.elems[k][j] * m.elems[k][k];
		for (int i = 0; i < n; i++)
			if (i != k)
				for (int j = 0; j < n; j++)
					if (j != k)
						m.elems[i][j] = m.elems[i][j] - m.elems[i][k] * m.elems[k][j];
		for (int i = 0; i < n; i++)
			if (i != k)
				m.elems[i][k] = -m.elems[i][k] * m.elems[k][k];
	}
	for (int k = n - 1; k >= 0; k--)
	{
		for (int j = 0; j < n; j++)
			if (js[k] != k)
				swap(m.elems[k][j], m.elems[js[k]][j]);
		for (int i = 0; i < n; i++)
			if (is[k] != k)
				swap(m.elems[i][k], m.elems[i][is[k]]);
	}
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
