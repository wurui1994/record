...
map_html_tags (const char *text, int size,
               //void (*mapfun) (struct taginfo *, char ***), char ***maparg,
               int flags,
...
look_for_tag:
		   ...
		   attributes.  Fill in the data and call the mapper function.  */
		{
            ...
            for (ii = 0; ii < taginfo.nattrs; ii++)
            {
                if(strcmp(taginfo.attrs[ii].name,"href") == 0
                        && tag_name_begin[0] == 'a')
                {
                    res = realloc (res, (num + 2) * sizeof (char *));
                    res[num] = strdup(taginfo.attrs[ii].value);
                    //puts(res[num]);
                    //printf ("%s=%s\n", taginfo.attrs[ii].name, taginfo.attrs[ii].value);
                    //++*(int *)arg;
                    res[++num] = NULL;
                }
            }
...
test_mapper (struct taginfo *taginfo, void *arg)
{
	...
	{
		if(strcmp(taginfo->attrs[i].name,"href") == 0)
        {
...
char ** get_urls_from_html(const char* filename)
{
	int size = 256;
	char *buffer;
	int length = 0;
	...
	char ** hrefs = map_html_tags (buffer, length, 0, NULL, NULL);
