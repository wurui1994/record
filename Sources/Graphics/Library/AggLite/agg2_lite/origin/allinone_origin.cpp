...
void draw_ellipse(rasterizer& ras,
                  double x,  double y,
                  double rx, double ry)
{
	int i;
...
int main()
{
	...
	rendering_buffer rbuf(buf, width, height, width * 3);

	// Create the renderer and the rasterizer
	renderer<span_rgb24> ren(rbuf);
	rasterizer ras;
	...
	for(i = 0; i < 10; i++) {
		...
		// Render
		ras.render(ren, rgba8(rand() & 0xFF,
		                      rand() & 0xFF,
		                      rand() & 0xFF,
		                      rand() & 0xFF));
	}
	...
	for(i = 0; i < 50; i++) {
		draw_ellipse(ras,
		             ...
		             random(3, 50),
		             random(3, 50));
		ras.render(ren, rgba8(rand() & 0x7F,
		                      rand() & 0x7F,
		                      rand() & 0x7F,
		                      (rand() & 0x7F) + 100));
	}
